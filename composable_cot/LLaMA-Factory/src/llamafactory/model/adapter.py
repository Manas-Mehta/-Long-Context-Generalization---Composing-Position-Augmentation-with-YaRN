...
def _setup_lora_tuning(
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        ...
        # Asymmetric LoRA: per-module rank/alpha overrides via PEFT's rank_pattern/alpha_pattern
        if getattr(finetuning_args, "parsed_lora_rank_pattern", None):
            peft_kwargs["rank_pattern"] = finetuning_args.parsed_lora_rank_pattern
            logger.info_rank0(f"Using asymmetric LoRA rank_pattern: {finetuning_args.parsed_lora_rank_pattern}")
        if getattr(finetuning_args, "parsed_lora_alpha_pattern", None):
            peft_kwargs["alpha_pattern"] = finetuning_args.parsed_lora_alpha_pattern
            logger.info_rank0(f"Using asymmetric LoRA alpha_pattern: {finetuning_args.parsed_lora_alpha_pattern}")
