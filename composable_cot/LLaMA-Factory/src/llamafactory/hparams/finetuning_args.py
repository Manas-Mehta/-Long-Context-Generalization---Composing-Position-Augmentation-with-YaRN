...
class LoraArguments:
    ...
    lora_rank_pattern: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Per-module LoRA rank overrides. Format: 'module:rank,module:rank'. "
                "Modules not listed use the default lora_rank. "
                "Example: 'q_proj:32,k_proj:32' gives rank 32 to Q/K, default rank to rest."
            )
        },
    )
    lora_alpha_pattern: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Per-module LoRA alpha overrides. Format: 'module:alpha,module:alpha'. "
                "Modules not listed use the default lora_alpha. "
                "Example: 'q_proj:64,k_proj:64' gives alpha 64 to Q/K, default alpha to rest."
            )
        },
    )
...
class FinetuningArguments(
    ...
    def __post_init__(self):
        ...
        # Parse per-module rank/alpha patterns: "q_proj:32,k_proj:32" -> {"q_proj": 32, "k_proj": 32}
        def parse_pattern(pattern_str):
            ...
            for item in pattern_str.split(","):
                ...
                if ":" in item:
                    ...
                    result[module.strip()] = int(value.strip())
            return result if result else None

        self.parsed_lora_rank_pattern: Optional[dict] = parse_pattern(self.lora_rank_pattern)
        self.parsed_lora_alpha_pattern: Optional[dict] = parse_pattern(self.lora_alpha_pattern)
